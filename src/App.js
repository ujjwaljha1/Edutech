...
// import LatestNote from './Pages/Notes/LatestNote';
...
//         <Route path='/addnote' element={<NoteInput/>}/>
//         <Route path='/latest' element={<LatestNote/>}/>
//       </Routes>
//       <Footer/>
...
import LatestNote from './Pages/Notes/LatestNote';
import UserProfile from './AdminPanel/UserProfile';
import FrustratingFantasticLoader from './FrustratingFantasticLoader';
...
function App() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setLoading(false);
    }, 4000); // Increased to 5 seconds for more loader visibility

    return () => clearTimeout(timer);
  }, []);

  if (loading) {
    return <FrustratingFantasticLoader />;
  }
  ...
  return (
    <div className="App">
      ...
      <Routes>
        ...
        <Route path="/courses" element={<HomePage />} />
        <Route path="/:slug" element={<CategoryPage />} />
        <Route path="/:slug/:contentSlug" element={<CategoryPage />} />
        <Route path='/careers' element={<CareerPage/>}/>
        <Route path="/Login" element={<Login />} />
        <Route path='/profile' element={<UserProfile/>}/>
        <Route path='/about' element={<AboutUs/>}/>
