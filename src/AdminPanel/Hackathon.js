...
const HackathonPage = () => {
  ...
  const fetchHackathons = async () => {
    try {
      const response = await axios.get(`${config.backendUrl}/api/hackathons`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`${config.backendUrl}/api/hackathons/${editingId}`, formData);
      } else {
        await axios.post(`${config.backendUrl}/api/hackathons`, formData);
      }
      fetchHackathons();
      resetForm();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${config.backendUrl}/api/hackathons/${id}`);
      fetchHackathons();
    } catch (error) {
