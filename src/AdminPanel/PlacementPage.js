...
const PlacementPage = () => {
  ...
  const fetchPlacements = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`${config.backendUrl}/api/placements`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`${config.backendUrl}/api/placements/${editingId}`, formData);
      } else {
        await axios.post(`${config.backendUrl}/api/placements`, formData);
      }
      fetchPlacements();
      resetForm();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${config.backendUrl}/api/placements/${id}`);
      fetchPlacements();
    } catch (error) {
