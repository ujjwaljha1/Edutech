...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.set([button, image, title], { autoAlpha: 0 });

      const tl = gsap.timeline({ defaults: { ease: 'power3.out' } });

      tl.fromTo(hero, { backgroundColor: '#ffffff' }, { backgroundColor: '#f8fafc', duration: 1 })
        .fromTo(title, { y: 50 }, { y: 0, autoAlpha: 1, duration: 0.8 }, '-=0.5')
        .fromTo(button, { y: 20 }, { y: 0, autoAlpha: 1, duration: 0.5 }, '-=0.3')
        .fromTo(image, { scale: 0.8, rotation: -5 }, { scale: 1, rotation: 0, autoAlpha: 1, duration: 0.7 }, '-=0.5');
  ...
  return (
    <div ref={scrollRef} className="smooth-scroll">
      <section ref={heroRef} className="relative min-h-screen flex items-center overflow-hidden bg-gradient-to-br from-blue-50 to-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col lg:flex-row items-center justify-between">
            <div ref={contentRef} className="lg:w-1/2 mb-12 lg:mb-0">
              <h1 ref={titleRef} className="text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight mb-6 text-blue-900">
                {/* Typewriter effect will be applied here */}
              </h1>
              <p className="text-xl text-blue-700 mb-8">
                Embark on a journey of knowledge and discover your potential.
              </p>
              <button
                ref={buttonRef}
                onClick={handleButtonClick}
                className="px-8 py-3 bg-blue-600 text-white rounded-full font-bold text-lg hover:bg-blue-700 transition duration-300 ease-in-out shadow-xl relative overflow-hidden group"
              >
            ...
            <div className="w-full lg:w-1/2 relative">
              <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-blue-200 to-blue-100 rounded-full filter blur-3xl opacity-30 animate-pulse"></div>
              <img
                ref={imageRef}
                src="https://cdn-icons-png.flaticon.com/256/12350/12350489.png"
                alt="Education Illustration"
                className="relative z-10 rounded-3xl shadow-2xl transform hover:rotate-3 transition-transform duration-300 mx-auto lg:ml-auto"
              />
            </div>
          </div>
        </div>
